...
#
#	pip3 install pyserial threading2
#
import serial
import threading
import time
...
################################################################################
	...
	# FHT80TF, "FenstersensorMultiraum",    "52FB", "7B"
	# FHT80TF, "FenstersensorNico",         "7D66", "C0"
	# FHT80TF, "FenstersensorLina",         "AD14", "1B"
	# FHT80TF, "FenstersensorKueche",       "9F63", "ED"
	# FHT80TF, "FenstersensorKellerbuero",  "5A33", "92"
	# FHT80TF, "FenstersensorKellertuere",  "005E", "4F"
	# FHT80TF, "FenstersensorHaustuere",    "B79D", "A0"
...
def parseFHT(msg):
	...
	if len(msg) == 12: # FHT80TF/Window
		...
		if housecode in FHT80TF: 
			...
			print("FHT80TF: %-12s %-22s (raw:%s)" % (FHT80TF[housecode], state, splitmsg))
			logger("FHT80TF: %-12s %-22s (raw:%s)" % (FHT80TF[housecode], state, splitmsg))
		else:
		...
		if housecode in FHT80:
			...
			print("FHT80:   %-12s %-22s %-5s (raw:%s)" % (FHT80[housecode], state, value, splitmsg))
			logger("FHT80:   %-12s %-22s %-5s (raw:%s)" % (FHT80[housecode], state, value, splitmsg))
		else:
...
def main():
	...
	try:
		while (True):
			...
			#print(cul_raw_message)
			msg = decode_hex(cul_raw_message.hex())[0]
